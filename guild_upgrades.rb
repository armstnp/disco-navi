...
module GW2
  ...
  class GuildUpgradeListHandler
    ...
    def handle(event)
      ...
      upgrades_by_type = upgrades_in_progress
        .collect { |u| u.values_at(:type, :name) }
        .group_by { |u| u[0] }
        .sort
        .collect do |kv|
          type, upgrades = kv
          "**#{type}**\n" + (upgrades.collect { |u| u[1] }.sort.collect { |u| "- #{u}" } * "\n")
        end * "\n\n"
  ...
  class GuildUpgradeInfoHandler
    ...
    def handle(event)
      ...
      matched_upgrades = upgrades_in_progress.deep_locate -> (key, value, obj) { key == :name && value.include?(upgrade_name) }
      if matched_upgrades.empty?
        event << "Couldn't find any upgrades similar to '#{upgrade_name}' in progress for '#{guild_name}'"
        return
      ...
      event.channel.send_embed do |embed|
        ...
        embed.add_field(name: 'Complete!', value: 'This upgrade is ready to build!') if item_costs.empty? && non_item_costs.empty?
        embed
      end
    end
  ...
  class GuildContributionsHandler
    ...
    def handle(event)
      ...
      GW2::API::ItemCatalogMany.request(ids: item_ids)
        .each do |item|
          item_id = item[:id]
          item_name = item[:name]
      ...
      event << (
        ...
        (contributions.values.collect { |c| "#{c[:name]} - #{c[:count]}" }.to_a * "\n"))
    end
  ...
  class GuildIngredientUpgradesHandler
    ...
    def handle(event)
      ...
      treasury_item_ids = treasury.collect { |i| i[:item_id] } * ","
      ...
      upgrade_ids = treasury_item[:needed_by].collect { |u| u[:upgrade_id] } * ","
      upgrades_by_id = GW2::API::GuildUpgradeCatalogMany
      ...
      event.channel.send_embed do |embed|
        ...
        embed.description = TextFormat::GradientProgressBar.new(
          ...
          max: total_needed).render
        embed.thumbnail = { url: item[:icon] }
        embed
      end

      has_enough, not_enough = treasury_item[:needed_by]
        .partition { |u| u[:count] <= treasury_item[:count] }
        .collect { |u_list| format_upgrade_list(u_list, upgrades_by_id) }
    ...
    def format_upgrade_list(upgrades, upgrades_by_id)
      upgrades
        .collect { |u| [u[:count], upgrades_by_id[u[:upgrade_id]][:name]] }
        .sort
        .collect { |u| "#{u[1]} :: #{u[0]}" }
        .join("\n")
    end
  ...
  class GuildReadyUpgradesHandler
    ...
    def handle(event)
      ...
      upgrades = GW2::API::GuildUpgradeCatalogMany.request(ids: upgrade_ids)
        .reject do |u|
      ...
      formatted_upgrades = upgrades
        .sort_by { |u| u.values_at(:missing_aether, :name) }
        .collect do |u|
          aether_info = ""
          aether_info = " (pending #{u[:missing_aether]} Aetherium)" if u[:missing_aether] > 0
          "#{u[:name]}#{aether_info}"
        end
        .join("\n")
      event << formatted_upgrades
